...
script("$uf.onclick = function () { return confirm('".($Qd?js_escape($Qd):'Naozaj?')."'); };","");}function
...
upload_error($m){$Nd=($m==UPLOAD_ERR_INI_SIZE?ini_get("upload_max_filesize"):0);return($m?'Súbor sa nepodarilo nahrať.'.($Nd?" ".sprintf('Maximálna povolená veľkosť súboru je %sB.',$Nd):""):'Súbor neexistuje.');}function
...
edit_form($Q,$o,$I,$Hg){global$b,$x,$pg,$m;$Xf=$b->tableName(table_status1($Q,true));page_header(($Hg?'Upraviť':'Vložiť'),$m,array("select"=>array($Q,$Xf)),$Xf);$b->editRowPrint($Q,$o,$I,$Hg);if($I===false)echo"<p class='error'>".'Žiadne riadky.'."\n";echo'<form action="" method="post" enctype="multipart/form-data" id="form">
...
h($g->error);}function
check_sqlite_name($B){global$g;$jc="db|sdb|sqlite";if(!preg_match("~^[^\\0]*\\.($jc)\$~",$B)){$g->error=sprintf('Prosím vyberte jednu z koncoviek %s.',str_replace("|",", ",$jc));return
false;}return
true;}function
create_database($k,$d){global$g;if(file_exists($k)){$g->error='Súbor existuje.';return
...
as$k){if(!@unlink($k)){$g->error='Súbor existuje.';return
false;}}return
true;}function
rename_database($B,$d){global$g;if(!check_sqlite_name($B))return
false;$g->__construct(":memory:");$g->error='Súbor existuje.';return@rename(DB,$B);}function
...
Min_DB;$vb=$b->credentials();if($g->connect($vb[0],$vb[1],$vb[2])){if(min_version(9,0,$g)){$g->query("SET application_name = 'Adminer'");if(min_version(9.2,0,$g)){$Qf['Reťazce'][]="json";$U["json"]=4294967295;if(min_version(9.4,0,$g)){$Qf['Reťazce'][]="jsonb";$U["jsonb"]=4294967295;}}}return$g;}return$g->error;}function
get_databases(){return
...
set_schema($pf,$h=null){global$g,$U,$Qf;if(!$h)$h=$g;$H=$h->query("SET search_path TO ".idf_escape($pf));foreach(types()as$T){if(!isset($U[$T])){$U[$T]=0;$Qf['Užívateľské typy'][]=$T;}}return$H;}function
...
driver_config(){$U=array();$Qf=array();foreach(array('Čísla'=>array("number"=>38,"binary_float"=>12,"binary_double"=>21),'Dátum a čas'=>array("date"=>10,"timestamp"=>29,"interval year"=>12,"interval day"=>28),'Reťazce'=>array("char"=>2000,"varchar2"=>4000,"nchar"=>2000,"nvarchar2"=>4000,"clob"=>4294967295,"nclob"=>4294967295),'Binárne'=>array("raw"=>2000,"long raw"=>2147483648,"blob"=>4294967295,"bfile"=>4294967296),)as$y=>$X){$U+=$X;$Qf[$y]=array_keys($X);}return
...
last_id(){global$g;return$g->last_id;}function
driver_config(){$U=array();$Qf=array();foreach(array('Čísla'=>array("long"=>3,"integer"=>5,"short"=>8,"byte"=>10,"double"=>20,"float"=>66,"half_float"=>12,"scaled_float"=>21),'Dátum a čas'=>array("date"=>10),'Reťazce'=>array("string"=>65535,"text"=>65535),'Binárne'=>array("binary"=>255),)as$y=>$X){$U+=$X;$Qf[$y]=array_keys($X);}return
...
as$B=>$Eb){$n=$o[$B];if(preg_match("~enum~",$n["type"])||like_bool($n)){$y=$sd[$B];$s--;echo"<div>".h($Eb)."<input type='hidden' name='where[$s][col]' value='".h($B)."'>:",(like_bool($n)?" <select name='where[$s][val]'>".optionlist(array(""=>"",'no','yes'),$Z[$y]["val"],true)."</select>":enum_input("checkbox"," name='where[$s][val][]'",$n,(array)$Z[$y]["val"],($n["null"]?0:null))),"</div>\n";unset($f[$B]);}elseif(is_array($C=$this->_foreignKeyOptions($_GET["select"],$B))){if($o[$B]["null"])$C[0]='('.'prázdne'.')';$y=$sd[$B];$s--;echo"<div>".h($Eb)."<input type='hidden' name='where[$s][col]' value='".h($B)."'><input type='hidden' name='where[$s][op]' value='='>: <select name='where[$s][val]'>".optionlist($C,$Z[$y]["val"],true)."</select></div>\n";unset($f[$B]);}}$s=0;foreach($Z
as$X){if(($X["col"]==""||$f[$X["col"]])&&"$X[col]$X[val]"!=""){echo"<div><select name='where[$s][col]'><option value=''>(".'kdekoľvek'.")".optionlist($f,$X["col"],true)."</select>",html_select("where[$s][op]",array(-1=>"")+$this->operators,$X["op"]),"<input type='search' name='where[$s][val]' value='".h($X["val"])."'>".script("mixin(qsl('input'), {onkeydown: selectSearchKeydown, onsearch: selectSearchSearch});","")."</div>\n";$s++;}}echo"<div><select name='where[$s][col]'><option value=''>(".'kdekoľvek'.")".optionlist($f,null,true)."</select>",script("qsl('select').onchange = selectAddRow;",""),html_select("where[$s][op]",array(-1=>"")+$this->operators),"<input type='search' name='where[$s][val]'></div>",script("mixin(qsl('input'), {onchange: function () { this.parentNode.firstChild.onchange(); }, onsearch: selectSearchSearch});"),"</div></fieldset>\n";}function
selectOrderPrint($pe,$f,$w){$qe=array();foreach($w
as$y=>$v){$pe=array();foreach($v["columns"]as$X)$pe[]=$f[$X];if(count(array_filter($pe,'strlen'))>1&&$y!="PRIMARY")$qe[$y]=implode(", ",$pe);}if($qe){echo'<fieldset><legend>'.'Zotriediť'."</legend><div>","<select name='index_order'>".optionlist(array(""=>"")+$qe,($_GET["order"][0]!=""?"":$_GET["index_order"]),true)."</select>","</div></fieldset>\n";}if($_GET["order"])echo"<div style='display: none;'>".hidden_fields(array("order"=>array(1=>reset($_GET["order"])),"desc"=>($_GET["desc"]?array(1=>1):array()),))."</div>\n";}function
...
true;}function
selectEmailPrint($Ub,$f){if($Ub){print_fieldset("email",'E-mail',$_POST["email_append"]);echo"<div>",script("qsl('div').onkeydown = partialArg(bodyKeydown, 'email');"),"<p>".'Odosielateľ'.": <input name='email_from' value='".h($_POST?$_POST["email_from"]:$_COOKIE["adminer_email"])."'>\n",'Predmet'.": <input name='email_subject' value='".h($_POST["email_subject"])."'>\n","<p><textarea name='email_message' rows='15' cols='75'>".h($_POST["email_message"].($_POST["email_append"]?'{$'."$_POST[email_addition]}":""))."</textarea>\n","<p>".script("qsl('p').onkeydown = partialArg(bodyKeydown, 'email_append');","").html_select("email_addition",$f,$_POST["email_addition"])."<input type='submit' name='email_append' value='".'Vložiť'."'>\n";echo"<p>".'Prílohy'.": <input type='file' name='email_files[]'>".script("qsl('input').onchange = emailFileChange;"),"<p>".(count($Ub)==1?'<input type="hidden" name="email_field" value="'.h(key($Ub)).'">':html_select("email_field",$Ub)),"<input type='submit' name='email' value='".'Odoslať'."'>".confirm(),"</div>\n","</div></fieldset>\n";}}function
...
';if($Td=="auth"){$tc=true;foreach((array)$_SESSION["pwds"]as$Pg=>$_f){foreach($_f[""]as$V=>$E){if($E!==null){if($tc){echo"<ul id='logins'>",script("mixin(qs('#logins'), {onmouseover: menuOver, onmouseout: menuOut});");$tc=false;}echo"<li><a href='".h(auth_url($Pg,"",$V))."'>".($V!=""?h($V):"<i>".'prázdne'."</i>")."</a>\n";}}}}else{$this->databasesPrint($Td);if($Td!="db"&&$Td!="ns"){$R=table_status('',true);if(!$R)echo"<p class='message'>".'Žiadne tabuľky.'."\n";else$this->tablesPrint($R);}}}function
databasesPrint($Td){}function
tablesPrint($S){echo"<ul id='tables'>",script("mixin(qs('#tables'), {onmouseover: menuOver, onmouseout: menuOut});");foreach($S
as$I){echo'<li>';$B=$this->tableName($I);if(isset($I["Engine"])&&$B!="")echo"<a href='".h(ME).'select='.urlencode($I["Name"])."'".bold($_GET["select"]==$I["Name"]||$_GET["edit"]==$I["Name"],"select")." title='".'Vypísať dáta'."'>$B</a>\n";}echo"</ul>\n";}function
